...
interface ParentDashboardProps {
  ...
  onBindSuccess: (newChildId: string) => void;
}
...
export const ParentDashboard: React.FC<ParentDashboardProps> = ({
  ...
  const handleBind = async (e: React.FormEvent) => {
    ...
    try {
      const data = await api.post<{ message: string; childId: string }>('/api/auth/parent/bind', {
        childUsername,
        bindCode,
      });
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-4xl mx-auto p-4">
      ...
      {!childId ? (
        // 未绑定状态
        <div className="card-jelly bg-white max-w-[500px] mx-auto w-full mt-6">
